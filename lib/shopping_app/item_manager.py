...
def items_list(self):   # Devuelve todas las instancias de Item que posees (de las cuales eres propietario).
    items = [item for item in Item.item_all() if item.owner == self]
    return items

def pick_items(self, number, quantity):   # Devuelve la cantidad especificada de las instancias de Item que posees y que corresponden al número.
...
def show_items(self):   # Muestra el estado del inventario de las instancias de Item que posees en formato de tabla con las columnas ["Número", "Nombre del producto", "Precio", "Cantidad"].
    ...
    print(tabulate(table_data, headers=["Número", "Nombre del producto", "Precio", "Cantidad"], tablefmt="grid"))    # Usa el módulo tabulate para mostrar el resultado en formato de tabla
...
def _stock(self):   # Devuelve el estado del inventario de las instancias de Item que posees.
    ...
    group_list = []
    for key, group in groupby(item_ls, key=lambda m: m.name):   # Agrupa las instancias de Item que devuelven el mismo valor por Item#name.
